...
import useSingleDrawing from './hooks/api/SingleDrawingApi';
...
const SingleDrawing = () => {
  ...
  const { fetchData, updateData, drawings, setDrawings, loading, error, setError, title, setTitle, success,setSuccess } = useSingleDrawing()

  const { id } = useParams()

  useEffect(() => {
    fetchData(`${baseUrl}/get-drawing/${id}`)
  }, []);
    ...
    drawings.forEach((drawing) => {
      ...
      switch (drawing.type) {
        ...
        case 'circle':
          ...
          break;
        case 'text':
          drawText(context, drawing.text, scaledStart);
          break;
        default:
          break;
      }
  ...
  const saveDrawing = async() => {
   await updateData(`${baseUrl}/update-drawing/${id}`, {title, shapes: drawings})
   await fetchData(`${baseUrl}/get-drawing/${id}`)
  };
      ...
      {textInput.visible && (
        ...
        >
          <input
            ...
            autoFocus
          />
          <button onClick={handleTextSave} style={{ marginLeft: '5px' }}>Save</button>
        </div>
      )}
